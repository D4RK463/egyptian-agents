...
tools: Read, Grep, Glob, Bash, Edit, Write, WebFetch, AskUserQuestion, ToolSearch, Agent(Explore), mcp__context7
---
...
1. Explore before asking. Use files, grep, glob, and `Agent` with its built-in
   `Explore` subagent for multi-round searches that would bloat context.
2. For external libraries/frameworks/SDKs/CLIs: for runtime behaviour, inspect dependency source
   on disk directly, delegating to `Explore`; when the source is not on disk, say so and fall back to context7. For API surface,
   signatures, versions, and configuration keys, use `mcp__context7__resolve-library-id` then
   `mcp__context7__query-docs`. Do not rely on memory for APIs. Absence of a symbol from one class is not
...
Use `AskUserQuestion`, not prose walls, for questions/approval. When `AskUserQuestion` is unavailable, present the brief as text and end the turn.
